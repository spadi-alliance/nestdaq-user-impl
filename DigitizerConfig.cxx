...
void DigitizerConfig::SetDefaultConfiguration(WaveDumpConfig_t *WDcfg) {
    int i, j;
...
*               recalculations.
*/
int DigitizerConfig::ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
		...
		for (i = 0; i < MAX_SET; i++)
		  {
		      ...
		      thr_file[i] = val;
		      WDcfg->thr_file[i] = val;
		  }
	      else
		{
		    WDcfg->Threshold[ch] = val;
		    thr_file[ch] = val;
		    WDcfg->thr_file[ch] = val;
		}
	      continue;
	}
...
int DigitizerConfig::Load_DAC_Calibration_From_Flash(int handle, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo) {
	...
	calibration_data = (float*)malloc(1 + VIRTUAL_PAGE_SIZE * sizeof(uint8_t));
	memset(calibration_data, 0, 1 + VIRTUAL_PAGE_SIZE * sizeof(uint8_t));
	...
	if (buffer[0] != 0xD) {
		printf("\nNo DAC Calibration data found in board flash.\n\n");
...
void DigitizerConfig::Save_DAC_Calibration_To_Flash(int handle, WaveDumpConfig_t WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo) {
	FLASH_API_ERROR_CODES err = FLASH_API_SUCCESS;
	uint8_t *buffer;
	int ch = 0;
	//float calibration_data[2*MAX_SET];
