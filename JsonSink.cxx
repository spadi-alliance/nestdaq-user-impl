...
using opt = nestdaq::JsonSink::OptionKey;
...
std::unique_ptr<fair::mq::Device> getDevice(FairMQProgOptions &config)
{
    ...
    return std::make_unique<nestdaq::JsonSink>();
}
...
//______________________________________________________________________________
bool JsonSink::CompressData(FairMQMessagePtr &rawMsg, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool JsonSink::CompressMultipartData(FairMQParts &rawMsgParts, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool JsonSink::HandleData(FairMQMessagePtr &msg, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool JsonSink::HandleDataMT(FairMQMessagePtr &msg, int index)
{
    (void)index;
...
//______________________________________________________________________________
bool JsonSink::HandleMultipartData(FairMQParts &msgParts, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool JsonSink::HandleMultipartDataMT(FairMQParts &msgParts, int index)
{
    (void)index;
...
void JsonSink::InitTask()
{
    ...
    if (fMultipart) {
        ...
        if ((fNThreads > 1) && (compressFormat != Compressor::Format::none)) {
            OnData(fInputDataChannelName, &JsonSink::HandleMultipartDataMT);
            fWorker = std::make_unique<TaskProcessorMT>(*this);
            LOG(debug) << " use multi-thread for compression";
        } else {
            OnData(fInputDataChannelName, &JsonSink::HandleMultipartData);
            LOG(debug) << " single thread";
        }
    } else {
        LOG(debug) << " register data handler for single message";
        if ((fNThreads > 1) && (compressFormat != Compressor::Format::none)) {
            OnData(fInputDataChannelName, &JsonSink::HandleDataMT);
            fWorker = std::make_unique<TaskProcessorMT>(*this);
            LOG(debug) << " use multi-thread for compression";
        } else {
            OnData(fInputDataChannelName, &JsonSink::HandleData);
            LOG(debug) << " single thread";
        }
    }
...
void JsonSink::PostRun()
{
    ...
    //fFile->Write(reinterpret_cast<char *>(&fJsonSinkTrailer), sizeof(JsonSinkTrailer::Trailer));
...
void JsonSink::PreRun()
{
    ...
    if (fWorker) {
        // FileUtil writes data compressed by worker threads of JsonSink.
    ...
    //fFile->Write(reinterpret_cast<char *>(&fJsonSinkHeader), sizeof(JsonSinkHeader::Header));
    //std::string str1 = "aa\n";
    //fFile->Write(str1.c_str(), str1.size());
}

//______________________________________________________________________________
bool JsonSink::WriteData(FairMQMessagePtr &msg, int index)
{
    (void)index;
...
//______________________________________________________________________________
bool JsonSink::WriteMultipartData(FairMQParts &msgParts, int index)
{
    (void)index;
	...
	for (int i = 0; i < v.size()/8; i++) {
	  ...
	  if (idata.head == AmQStrTdc::Data::Heartbeat) {
	    ...
	    ss << std::dec;
	    ss << "{\"SimTrackerHits\":[{\"cellID\":" << idata.hrch << ",\"eDep\":" << idata.hrtot << ".0,\"time\":" << idata.hrtdc << ".0,\"pathLength\":0.0,\"quality\":0,\"position\":[0.0,0.0,0.0],\"momentum\":[0.0,0.0,0.0]}]}" << std::endl;
