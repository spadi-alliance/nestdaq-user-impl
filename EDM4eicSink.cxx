...
using opt = nestdaq::EDM4eicSink::OptionKey;
...
std::unique_ptr<fair::mq::Device> getDevice(FairMQProgOptions &config)
{
    ...
    return std::make_unique<nestdaq::EDM4eicSink>();
}
...
//______________________________________________________________________________
bool EDM4eicSink::CompressData(FairMQMessagePtr &rawMsg, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool EDM4eicSink::CompressMultipartData(FairMQParts &rawMsgParts, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool EDM4eicSink::HandleData(FairMQMessagePtr &msg, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool EDM4eicSink::HandleDataMT(FairMQMessagePtr &msg, int index)
{
    (void)index;
...
//______________________________________________________________________________
bool EDM4eicSink::HandleMultipartData(FairMQParts &msgParts, int index)
{
    if (fStopRequested) {
        return false;
    }
...
//______________________________________________________________________________
bool EDM4eicSink::HandleMultipartDataMT(FairMQParts &msgParts, int index)
{
    (void)index;
...
//______________________________________________________________________________
void EDM4eicSink::Init() {}

//______________________________________________________________________________
void EDM4eicSink::InitTask()
{
    ...
    if (fMultipart) {
      //LOG(debug) << " register data handler for multipart message";
        ...
        //    OnData(fInputDataChannelName, &EDM4eicSink::HandleMultipartDataMT);
        //    fWorker = std::make_unique<TaskProcessorMT>(*this);
        //    LOG(debug) << " use multi-thread for compression";
        //} else {
            OnData(fInputDataChannelName, &EDM4eicSink::HandleMultipartData);
            LOG(debug) << " single thread";
	    //}
    } else {
      //LOG(debug) << " register data handler for single message";
        //if ((fNThreads > 1) && (compressFormat != Compressor::Format::none)) {
        //    OnData(fInputDataChannelName, &EDM4eicSink::HandleDataMT);
        //    fWorker = std::make_unique<TaskProcessorMT>(*this);
        //    LOG(debug) << " use multi-thread for compression";
        //} else {
            OnData(fInputDataChannelName, &EDM4eicSink::HandleData);
            LOG(debug) << " single thread";
	    //}
    }
...
void EDM4eicSink::PostRun()
{
    ...
    //fFile->Write(reinterpret_cast<char *>(&fEDM4eicSinkTrailer), sizeof(EDM4eicSinkTrailer::Trailer));
...
void EDM4eicSink::PreRun()
{
    ...
    if (fWorker) {
        // FileUtil writes data compressed by worker threads of EDM4eicSink.
    ...
    //fFile->Write(reinterpret_cast<char *>(&fEDM4eicSinkHeader), sizeof(EDM4eicSinkHeader::Header));
    //std::string str1 = "aa\n";
    //fFile->Write(str1.c_str(), str1.size());
}

//______________________________________________________________________________
bool EDM4eicSink::WriteData(FairMQMessagePtr &msg, int index)
{
    (void)index;
